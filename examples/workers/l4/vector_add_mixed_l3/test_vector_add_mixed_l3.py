...
REMOTE_ORCH_TARGET = "examples.workers.l4.vector_add_mixed_l3.test_vector_add_mixed_l3:remote_l3_group_orch"
ELEMENTS = 128 * 128
...
def _tensor_from_shm(shm: shared_memory.SharedMemory) -> Tensor:
    buf = shm.buf
    assert buf is not None
    data_ptr = ctypes.addressof(ctypes.c_char.from_buffer(buf))
    return Tensor.make(data_ptr, (ELEMENTS,), DataType.FLOAT32)


def _make_local_group(values: tuple[float, float, float, float]) -> tuple[list[shared_memory.SharedMemory], list[Any], TaskArgs, dict[str, tuple[Any, float]]]:
    ...
    for index, value in enumerate(initial_values):
        ...
        view = FloatArray.from_buffer(shm.buf)
...
def main() -> int:
    ...
    try:
        ...
        for handle, array in zip(
            remote_handles,
            (_make_array(6.0), _make_array(7.0), _make_array(0.0), _make_array(8.0), _make_array(9.0), _make_array(0.0)),
        ):
